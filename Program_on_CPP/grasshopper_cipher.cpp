...
const std::vector<unsigned char> Pi = {
    ...
    0x39, 0x4B, 0x63, 0xB6};
const std::vector<unsigned char> reverse_Pi = {
    ...
    0x00, 0x4C, 0xD7, 0x74};
const std::vector<int> lagua_table = {
...
std::vector<int> lagua_table2 = {
    ...
    255};
const std::vector<unsigned char> l_vec = {148, 32,  133, 16, 194, 192, 1,   251,
...
void print_block(std::vector<unsigned char> a, int size = 16) {
  int i = 0;
  for (i = 0; i < size; i++) {
    std::cout << std::hex << ((unsigned int)(a[i] & 0b11110000) >> 4)
              << (unsigned int)(a[i] & 0b00001111);
  }
  std::cout << '\n';
}
...
unsigned char GOST_Kuz_GF_mul(unsigned char a, unsigned char b) {
    ...
    //    std::distance(lagua_table.begin(),
    //                  std::find(lagua_table.begin(), lagua_table.end(), a));
    int stepen_b =  lagua_table2[b - 1];
    //    std::distance(lagua_table.begin(),
    //                  std::find(lagua_table.begin(), lagua_table.end(), b));
...
void Encript_File(std::string file_name, std::string main_key) {
  ...
  std::filesystem::rename(file_name + ".crypto", file_name);
}
...
//   // Encript_File("Screenshot 2025-06-22 201626.png", main_key);
//   // Decript_File("Screenshot 2025-06-22 201626.png", main_key);
//   return 0;
// }

int main() {
  std::cout << "Преобразование S\n";
  std::vector<unsigned char> blk1 = {0xff, 0xee, 0xdd, 0xcc, 0xbb, 0xaa,
                                     0x99, 0x88, 0x11, 0x22, 0x33, 0x44,
                                     0x55, 0x66, 0x77, 0x00};
  std::vector<unsigned char> result = GOST_Kuz_S(blk1);
  print_block(result);
  result = GOST_Kuz_S(result);
  print_block(result);
  result = GOST_Kuz_S(result);
  ...
  std::cout << "\nПреобразование R\n";
  std::vector<unsigned char> blk2 = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                     0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                     0x00, 0x00, 0x01, 0x00};
  result = GOST_Kuz_R(blk2);
  print_block(result);
  ...
  std::cout << "\nПреобразование L\n";
  std::vector<unsigned char> blk3 = {0x64, 0xa5, 0x94, 0x00, 0x00, 0x00,
                                     0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                     0x00, 0x00, 0x00, 0x00};
  std::vector<unsigned char> blk4 = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                     0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                     0x00, 0x00, 0x00, 0x01};
  result = GOST_Kuz_L(blk3);
  print_block(result);
  result = GOST_Kuz_L(result);
  print_block(result);
  result = GOST_Kuz_L(result);
  ...
  int i = 0;
  for (i = 0; i < 32; i++) {
    print_block(iter_C[i]);
  }

  std::cout << "\nИтерационные ключи\n";
  std::vector<unsigned char> key_1 = {0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd,
                                      0xee, 0xff, 0x00, 0x11, 0x22, 0x33,
                                      0x44, 0x55, 0x66, 0x77};
  std::vector<unsigned char> key_2 = {0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54,
  ...
  std::cout << "\nЗашифрованный блок\n";
  std::vector<unsigned char> a = {0x11, 0x22, 0x33, 0x44, 0x55, 0x66,
                                  0x77, 0x00, 0xff, 0xee, 0xdd, 0xcc,
                                  0xbb, 0xaa, 0x99, 0x88};
  std::vector<unsigned char> encriptBlok = GOST_Kuz_Encript(a, key_1, key_2);
  print_block(encriptBlok);

  std::cout << "\nОбратное преобразование S\n";
  std::vector<unsigned char> blk5 = {0x23, 0xae, 0x65, 0x63, 0x3f, 0x84,
                                     0x2d, 0x29, 0xc5, 0xdf, 0x52, 0x9c,
                                     0x13, 0xf5, 0xac, 0xda};
  result = GOST_Kuz_reverse_S(blk5);
  print_block(result);
  result = GOST_Kuz_reverse_S(result);
  print_block(result);
  result = GOST_Kuz_reverse_S(result);
  ...
  std::vector<unsigned char> blk6 = {0x0d, 0x64, 0xa5, 0x94, 0x00, 0x00,
                                     0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                     0x00, 0x00, 0x00, 0x00};
  result = GOST_Kuz_reverse_R(blk6);
  print_block(result);
  ...
  std::vector<unsigned char> blk7 = {0xe6, 0xa8, 0x09, 0x4f, 0xee, 0x0a,
                                     0xa2, 0x04, 0xfd, 0x97, 0xbc, 0xb0,
                                     0xb4, 0x4b, 0x85, 0x80};
  result = GOST_Kuz_reverse_L(blk7);
  print_block(result);
  result = GOST_Kuz_reverse_L(result);
  print_block(result);
  result = GOST_Kuz_reverse_L(result);
  ...
  std::cout << "\nРасшифрованный блок\n";
  std::vector<unsigned char> decriptBlok = GOST_Kuz_Decript(encriptBlok, key_1,
  key_2); print_block(decriptBlok);
  }